...
class Nilskripsi_kuji_model extends CI_Model
{
    ...
    {
        $this->db->insert('nilskripsi_kuji_dosen', $data);
    }

    public function get_dosen_data($id, $username)
    {
        $this->db->where('id_nilskripsi_kuji', $id);
        $this->db->where('username', $username);
        return $this->db->get('nilskripsi_kuji_dosen')->row();
    }
    ...
    {
        $this->db->select('nilskripsi_kuji.*, dfskripsi.nama_mhs_skripsi, dfskripsi.nim, dfskripsi.prodi_skripsi, dfskripsi.judul_skripsi, nilskripsi_kuji.id_nilskripsi_kuji');
        $this->db->from($table);
        $this->db->join('dfskripsi', 'dfskripsi.id_skripsi = nilskripsi_kuji.id_skripsi');
        $this->db->where($where);
        return $this->db->get();
    ...
    public function ambil_kuji_nim($nim)
    {
        $this->db->select('*');
        $this->db->from('nilskripsi_kuji');
    ...
    public function get_pengisian_count($id)
    {
        ...
        $this->db->where('id_nilskripsi_kuji', $id);
        return $this->db->get('nilskripsi_kuji')->row()->pengisian_count;
    }
    ...
    public function increment_pengisian_count($id)
    {
        ...
        $this->db->where('id_nilskripsi_kuji', $id);
        $this->db->update('nilskripsi_kuji');
    }
}
